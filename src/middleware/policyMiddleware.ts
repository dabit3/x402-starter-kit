...
export class PaymentPolicyMiddleware {
  ...
  constructor(config: PolicyConfig) {
    ...
    // Validate numeric config upfront so we fail fast on misconfiguration.
    if (config.maxPerTransaction !== undefined) usdcToAtomic(config.maxPerTransaction);
    if (config.maxPerHour !== undefined) usdcToAtomic(config.maxPerHour);
    if (config.maxPerDay !== undefined) usdcToAtomic(config.maxPerDay);
 ...
 *
 * Input must be a numeric string with up to 6 decimal places.
 */
export function usdcToAtomic(usdcString: string): bigint {
  const trimmed = usdcString.trim();

  // Only allow simple decimal strings with up to 6 digits after the decimal point.
  // Examples: "1", "1.0", "0.10", "123.123456"
  if (!/^\d+(\.\d{0,6})?$/.test(trimmed)) {
    throw new Error(
      `Invalid USDC amount "${usdcString}" (expected a numeric string with up to 6 decimals)`
    );
  }
